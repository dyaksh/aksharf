...
export default function ServicesSection() {
  ...
  return (
    <section id="services" className="bg-[#f8f3ed] relative overflow-hidden" ref={sectionRef}>
            ...
            {headingWords.map((word, i) => (
              ...
              >
                {word === '360°' || word === 'FF&E' || word === 'support,' ? (
                  <span className="text-[#5d2c86]">{word}</span>
                ) : word}
              </motion.span>
            ))}
          </h2>
      ...
      {/* ─────────────── 3. PROCESS STEPS ─────────────── */}
      <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8 pb-14 lg:pb-20">
        <div ref={processRef}>
          <div className="text-center mb-10">
            <motion.p
              className="text-xs tracking-[0.3em] text-[#5d2c86] mb-3 font-sans-body font-semibold"
              initial={{ opacity: 0, letterSpacing: '0.6em' }}
              whileInView={{ opacity: 1, letterSpacing: '0.3em' }}
              viewport={{ once: true }}
            ...
            <motion.h3
              className="text-2xl sm:text-3xl lg:text-4xl font-bold font-serif-display text-[#000]"
            ...
            >
              Four steps.{' '}
              <span className="text-[#5d2c86]">Zero guesswork.</span>
            </motion.h3>
          </div>
                    ...
                    {index < processSteps.length - 1 && (
                      <div className="absolute top-8 lg:top-9 left-[calc(50%+28px)] right-0 z-0 flex items-center">
                        <div className="w-full relative h-[2px]">
                          <div className="absolute inset-0 bg-[#000]/8 rounded-full" />
                          <motion.div
                            className="absolute inset-y-0 left-0 rounded-full bg-[#5d2c86]"
                            initial={{ scaleX: 0 }}
                        ...
                        <motion.div
                          initial={{ opacity: 0 }}
                          animate={isProcessInView ? { opacity: 0.5 } : {}}
                          transition={{ delay: 1.8 + index * 0.25 }}
                        >
                          <div className="w-0 h-0 border-t-[4px] border-t-transparent border-b-[4px] border-b-transparent border-l-[6px] border-l-[#5d2c86] -ml-0.5" />
                        </motion.div>
                      </div>
                    )}
                    <div className="relative mb-4">
                      <motion.div
                        className="w-14 h-14 lg:w-[72px] lg:h-[72px] rounded-2xl bg-white border border-[#000]/8 flex items-center justify-center shadow-sm relative z-10"
                        initial={{ scale: 0.8, opacity: 0 }}
                        whileInView={{ scale: 1, opacity: 1 }}
                        viewport={{ once: true }}
                        transition={{ duration: 0.5, delay: 0.3 + index * 0.15 }}
                      >
                        <Icon className="w-6 h-6 lg:w-7 lg:h-7 text-[#5d2c86]" />
                      </motion.div>
                      <motion.div
                        className="absolute inset-0 rounded-2xl"
                        style={{ boxShadow: '0 0 20px rgba(93,44,134,0.2), 0 0 40px rgba(93,44,134,0.08)' }}
                        initial={{ opacity: 0, scale: 0.9 }}
                        animate={isProcessInView ? { opacity: [0, 0.5, 0], scale: [0.9, 1.15, 0.9] } : {}}
                        transition={{ duration: 2.5, delay: 0.5 + index * 0.2, repeat: Infinity, repeatDelay: 2, ease: 'easeInOut' }}
                      />
                    </div>
                    <motion.span className="text-[10px] font-sans-body font-bold tracking-[0.2em] text-[#5d2c86] mb-2" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.4 + index * 0.15 }}>
                      STEP 0{index + 1}
                    </motion.span>
                    <h4 className="text-sm lg:text-base font-bold text-[#000] font-sans-body mb-2">{step.title}</h4>
                    <p className="text-[11px] lg:text-xs text-[#000]/45 leading-relaxed font-sans-body">{step.description}</p>
                  </div>
                </RevealOnScroll>
              );
            })}
                  ...
                  <div className="flex gap-4">
                    <div className="flex flex-col items-center">
                      <div className="relative">
                        <div className="w-12 h-12 rounded-xl bg-white border border-[#000]/8 flex items-center justify-center shadow-sm relative z-10">
                          <Icon className="w-5 h-5 text-[#5d2c86]" />
                        </div>
                        <motion.div className="absolute inset-0 rounded-xl" style={{ boxShadow: '0 0 16px rgba(93,44,134,0.2)' }} initial={{ opacity: 0, scale: 0.9 }} animate={isProcessInView ? { opacity: [0, 0.5, 0], scale: [0.9, 1.1, 0.9] } : {}} transition={{ duration: 2.5, delay: 0.3 + index * 0.2, repeat: Infinity, repeatDelay: 2, ease: 'easeInOut' }} />
                      </div>
                      {index < processSteps.length - 1 && (
                        <div className="relative w-[2px] h-10 my-1">
                          <div className="absolute inset-0 bg-[#000]/8 rounded-full" />
                          <motion.div className="absolute inset-x-0 top-0 bottom-0 rounded-full bg-[#5d2c86]" initial={{ scaleY: 0 }} animate={isProcessInView ? { scaleY: 1 } : {}} transition={{ duration: 0.6, delay: 0.6 + index * 0.2 }} />
                        </div>
                      )}
                    </div>
                    <div className="pb-6">
                      <span className="text-[9px] font-sans-body font-bold tracking-[0.2em] text-[#5d2c86]">STEP 0{index + 1}</span>
                      <h4 className="text-sm font-bold text-[#000] font-sans-body mt-0.5 mb-1">{step.title}</h4>
                      <p className="text-xs text-[#000]/45 leading-relaxed font-sans-body">{step.description}</p>
                    </div>
                  </div>
                </RevealOnScroll>
              );
